// Validacion del formulario de usuarios del admin.
// Lo usan tanto usuario-nuevo.html como usuario-editar.html, porque tienen los mismos campos.
// Reutiliza el mismo algoritmo de RUN y las mismas reglas que assets/js/registro.js
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var campoTipoUsuario = document.getElementById('tipo-usuario');
    var campoNombre = document.getElementById('nombre');
    var campoApellidos = document.getElementById('apellidos');
    var campoCorreo = document.getElementById('correo');
    var campoFechaNacimiento = document.getElementById('fecha-nacimiento');
    ...
    campoRegion.addEventListener('change', function () {
        var regionElegida = REGIONES_COMUNAS.find(function (item) {
    ...
    function validarRun() {
        ...
        if (valor === '') {
            mostrarError(campoRun, 'Ingresa el RUN del usuario.');
            return false;
        }
    ...
    // Valida el tipo de usuario: requerido
    function validarTipoUsuario() {
        if (campoTipoUsuario.value === '') {
            mostrarError(campoTipoUsuario, 'Selecciona un tipo de usuario.');
            return false;
        }

        quitarError(campoTipoUsuario);
        return true;
    }
    ...
    function validarNombre() {
        ...
        if (valor === '') {
            mostrarError(campoNombre, 'Ingresa el nombre.');
            return false;
        }
        if (valor.length > 50) {
    ...
    function validarApellidos() {
        ...
        if (valor === '') {
            mostrarError(campoApellidos, 'Ingresa los apellidos.');
            return false;
        }
        if (valor.length > 100) {
    ...
    // Valida el correo: requerido, maximo 100 caracteres y dominio permitido
    function validarCorreo() {
        var valor = campoCorreo.value.trim();

        if (valor === '') {
            mostrarError(campoCorreo, 'Ingresa el correo electronico.');
            return false;
        }
        if (valor.length > 100) {
    ...
    function validarRegion() {
        if (campoRegion.value === '') {
            mostrarError(campoRegion, 'Selecciona una región.');
            return false;
        }
    ...
    function validarComuna() {
        if (campoComuna.value === '') {
            mostrarError(campoComuna, 'Selecciona una comuna.');
            return false;
        }
    ...
    function validarDireccion() {
        ...
        if (valor === '') {
            mostrarError(campoDireccion, 'Ingresa la dirección.');
            return false;
        }
        if (valor.length > 300) {
    ...
    formulario.addEventListener('submit', function (evento) {
        ...
        var tipoUsuarioValido = validarTipoUsuario();
        ...
        var todoValido = runValido && tipoUsuarioValido && nombreValido && apellidosValidos &&
            correoValido && regionValida && comunaValida && direccionValida;
        ...
        if (todoValido) {
            ...
            formulario.dispatchEvent(new CustomEvent('usuario-guardado'));
        } else {
            mensajeConfirmacion.classList.remove('visible');
        }
    });
